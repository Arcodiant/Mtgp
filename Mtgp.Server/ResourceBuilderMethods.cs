...
public partial class ResourceBuilder
{
	...
	public ResourceBuilder RenderPipeline(out Task<RenderPipelineHandle> task, ShaderStageInfo[] ShaderStages, VertexInputInfo VertexInput, FragmentAttribute[] FragmentAttributes, Rect3D? Viewport, Rect3D[] Scissors, int[] AlphaIndices, PolygonMode PolygonMode, string? reference = null)
		=> this.Add(new CreateRenderPipelineInfo(ShaderStages, VertexInput, FragmentAttributes, Viewport, Scissors, AlphaIndices, PolygonMode, reference), id => new RenderPipelineHandle(id), out task);
