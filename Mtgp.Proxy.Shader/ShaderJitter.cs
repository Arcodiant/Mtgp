    ...
    : ShaderExecutor
{
    ...
    private const int pushConstantsIndex = 5;
    private delegate void ExecuteDelegate(Span<byte> input, Span<byte> output, SpanCollection bufferAttachments, SpanCollection imageAttachments, SpanCollection imageDimensionCollection, Span<byte> pushConstants);
    ...
    public override void Execute(ImageState[] imageAttachments, Memory<byte>[] bufferAttachments, Span<byte> pushConstants, Span<byte> input, Span<byte> output)
    {
        ...
        this.execute(input, output, bufferCollection, imageCollection, imageDimensionCollection, pushConstants);
    }
