...
public class RenderPipelineTests
{
	...
	public void ShouldSetColours()
	{
		...
		var vertexOutputMappings = new ShaderIoMappingsBuilder()
										.AddBuiltin(ShaderType.Int(4), Builtin.PositionX)
										.AddBuiltin(ShaderType.Int(4), Builtin.PositionY)
										.AddLocation(ShaderType.VectorOf(ShaderType.Float(4), 3), 0)
										.AddLocation(ShaderType.VectorOf(ShaderType.Float(4), 3), 1)
										.Build();

		var fragmentOutputMappings = new ShaderIoMappingsBuilder()
										.AddLocation(ShaderType.Int(4), 0)
										.AddLocation(ShaderType.VectorOf(ShaderType.Float(4), 3), 1)
										.AddLocation(ShaderType.VectorOf(ShaderType.Float(4), 3), 2)
										.Build();
		...
		var shaderStages = new Dictionary<ShaderStage, IShaderExecutor>
		{
			[ShaderStage.Vertex] = new MockShaderExecutor((images, buffers, input, output) =>
			{
				...
				new BitWriter(vertexOutputMappings.GetBuiltin(output, Builtin.PositionX)).Write(vertexIndex == 0 ? 0 : 9);
				new BitWriter(vertexOutputMappings.GetBuiltin(output, Builtin.PositionY)).Write(vertexIndex == 0 ? 0 : 0);
			}, vertexInputMappings, vertexOutputMappings),
			[ShaderStage.Fragment] = new MockShaderExecutor((images, buffers, input, output) =>
			{
			}, vertexOutputMappings, fragmentOutputMappings)
		...
		var frameBufferImages = new ImageState[]
		{
			new(new(10, 1, 1), ImageFormat.T32_SInt),
			new(new(10, 1, 1), ImageFormat.R32G32B32_SFloat),
			new(new(10, 1, 1), ImageFormat.R32G32B32_SFloat)
		};
